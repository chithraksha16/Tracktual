...
const ParticularTask = () => {
  ...
  return (
    ...
    <div className="h-auto pb-15">
      ...
      <div className="flex flex-wrap sm:justify-start md:justify-center justify-center  sm:px-20 px-2 gap-5 md:gap-10 mt-15">
        {pDate.map((pdates)=>pdates.entries.map((entries,idx)=>(
          <div key={idx} className="sm:w-sm  w-full sm:h-auto h-52 sm:px-10 px-4 border border-white/60 rounded-3xl space-y-3 py-2 bg-radial-[at_90%_10%] from-20% from-black to-[#3B022B]    ">
